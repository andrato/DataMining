...
-->
$w_1 = \frac{\sum\limits_{i=1}^{m}(y_ix_i) -\frac{\sum\limits_{i=1}^{m}y_i\sum\limits_{i=1}^{m}x_i}{m}}{\sum\limits_{i=1}^{m}x_i^2 - \frac{(\sum\limits_{i=1}^{m}x_i)^2}{m}}$  **(B)**
...
Stochastic Gradient Descent is used when dealing with large size data (e.g. m = 1e20). Unlike the batch gradient descent which goes through all the cases,
...
### e) **Adjusted R Square** \
- the R-square is adjusted based on the complexity of the model. R-squared value goes up if more variables are included, even if the are redundant (don't contribute
